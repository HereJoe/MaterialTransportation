...
# CALL findCombination4MultiTarget ('Device A','Device E,Device G',3, FALSE, TRUE);

DELIMITER //
DROP PROCEDURE IF EXISTS findCombination4MultiTarget;
CREATE PROCEDURE findCombination4MultiTarget(
...
BEGIN
	...
	DECLARE combine_count INT default 0;
	...
	END IF;
	# def log type
	SET log_type = concat('findCombination4MultiTarget(',source_name,',',target_names,',',combine_max,',',enable_log,')');
	...
	INSERT INTO temp_path (current_node_id,type,comulative_cost,depth,node_ids,node_names,node_costs,edge_costs) SELECT
	id,type,cost,0,id,concat(LEFT(name, name_cut)),cost,0 FROM devices WHERE id = source_id AND type='Source';
	IF enable_log THEN
	...
	ltarget: LOOP
		...
		SET path_count=0;
		SELECT COUNT(*) INTO path_count FROM temp_path WHERE current_node_id=target_id AND finish=1 ORDER BY comulative_cost ASC, depth ASC LIMIT combine_max;
		IF combine_count*path_count<combine_max AND NOT queue_empty THEN
			que:LOOP
				...
				IF n_id=target_id THEN
					...
					IF enable_log THEN
						INSERT INTO log (type,message) VALUE (log_type,concat('visit target.(',combine_count,')'));
					END IF;
				...
				ELSE
					...
					cnr:LOOP
						...
						IF c_count=1 THEN
							...
							node_costs=concat(node_costs,',',c_cost),edge_costs=concat(edge_costs,',',e_cost)
							WHERE current_node_id=n_id;
						# insert for the first n-1 children
						ELSE
							INSERT INTO temp_path (current_node_id,type,comulative_cost,depth,node_ids,node_names,node_costs,edge_costs)
							SELECT c_id,c_type,comulative_cost+c_cost+e_cost,depth+1,concat(node_ids,',',c_id),concat(node_names,'->',LEFT(c_name, name_cut)),concat(node_costs,',',c_cost),concat(edge_costs,',',e_cost)
	...
	SET @strSql = CONCAT('INSERT INTO path (batch_id,source,target,total_cost,total_node,path,created_by) SELECT
		',batchId,',',source_id,',',target_id,',comulative_cost,depth+1,node_names,"findPath4MultiTarget" FROM temp_path WHERE current_node_id IN (',target_ids,') ORDER BY comulative_cost ASC, depth ASC LIMIT ',total_path_count);
	...
	SELECT batchId;
	IF print_path THEN
		SET @strSql = CONCAT('SELECT current_node_id,comulative_cost,node_names,node_costs,edge_costs FROM temp_path WHERE current_node_id IN (',target_ids,') ORDER BY comulative_cost ASC, depth ASC LIMIT ',total_path_count);
		PREPARE preparedStatement FROM @strSql;
		EXECUTE preparedStatement;
	END IF;
