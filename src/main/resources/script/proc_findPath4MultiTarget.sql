...
CALL findPath4MultiTarget ('Device A','Device E,Device G',3, FALSE, TRUE);

DELIMITER //
DROP PROCEDURE IF EXISTS findPath4MultiTarget;
CREATE PROCEDURE findPath4MultiTarget(
...
BEGIN
	...
	END IF;
	# def log type
	SET log_type = concat('findPath4MultiTarget(',source_name,',',target_names,',',path_max,',',enable_log,')');
	...
	IF print_path THEN
		SELECT @row_num:=@row_num+1 AS 'path #', p.path as 'path', p.total_cost as 'path cost' FROM path p,(SELECT @row_num := 0) r WHERE p.batch_id = batchId ORDER BY p.total_cost LIMIT path_max;
	END IF;
END //
DELIMITER ;